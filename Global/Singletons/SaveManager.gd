...
func save_game(saveName:String) -> bool:
    ...
    var isOk:bool = API.save_map(_EditableSave)
    isOk = isOk and _EditableSave.Save(SAVE_FOLDER + saveName + ".db")
...
func add_empty_map_to_save(folderPath:String, saveName:String, mapName:String) -> bool:
    ...
    if(not TempSave.open()):
        return false
    var isOk :=TempSave.set_new_empty_map(mapName)
    isOk = isOk and TempSave.Save()
    return isOk
